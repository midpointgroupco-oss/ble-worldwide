-- ============================================================
-- BLE WORLDWIDE — SUPABASE SCHEMA (FIXED)
...
create policy "Parents can read own child"
  on public.students for select
...
create policy "Anyone authenticated can read grade levels"
...
create policy "Admins and teachers access enrollments"
...
create policy "Parents read own child enrollments"
...
create policy "Auth users can read assignments"
...
create policy "Teachers manage submissions for their courses"
...
create policy "Admins access all attendance"
  on public.attendance for all
  using (
    exists (
      select 1 from public.profiles p
...
create policy "Parents read child attendance"
...
create policy "Auth users read schedule"
  on public.schedule_events for select
  using (auth.role() = 'authenticated');

create policy "Admins and teachers manage schedule"
...
create policy "Auth users read announcements"
...
create policy "Parents access own billing"
  on public.billing for select
  using (parent_id = auth.uid());

create policy "Admins access all billing"
